...
public class RAGController {
    ...
    public RAGController(@Qualifier("chatMemoryChatClientRag") ChatClient ollamaChatClient, @Qualifier("webSearchRAGChatClient") ChatClient webSearchchatClient, VectorStore vectorStore) {
...
//                .collect(Collectors.joining(System.lineSeparator()));
        String answer = ollamaChatClient.prompt()
...
//        SearchRequest searchRequest =
//                SearchRequest.builder().query(message).topK(2).similarityThreshold(0.5).build();
//        List<Document> similarDocs =  vectorStore.similaritySearch(searchRequest);
//        String similarContext = similarDocs.stream()
//                .map(Document::getText)
//                .collect(Collectors.joining(System.lineSeparator()));
